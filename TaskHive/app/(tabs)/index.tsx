...
export default function HomeScreen() {
  ...
  const [longPressedBoardId, setLongPressedBoardId] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    if (
      ...
      notifications[0].id !== prevNotificationId.current
    ) {
  ...
  useEffect(() => {
    if (selectedWorkspace) {
      ...
      console.log(
        "HomeScreen: Current boards state for workspace",
        selectedWorkspace.id,
        ":",
        JSON.stringify(fetchedBoards, null, 2)
      );
    }
  }, [selectedWorkspace]);
  ...
  useEffect(() => {
    if (params.board) {
      try {
        ...
        console.log(
          "HomeScreen: Received updated board:",
        ...
        setBoards((prev) => {
          ...
          console.log(
            "HomeScreen: Updated boards state:",
  ...
  const handleCreateBoard = async () => {
    ...
    setRecents((prev) => [
      {
        id: `recent-${Date.now()}`,
        message: `"${newBoard.title}" board created.`,
        timestamp: Date.now(),
      },
      ...prev,
    ]);
    ...
    if (boardToDelete !== null) {
      ...
      if (deletedBoard) {
        setRecents((prev) => [
          {
            id: `recent-${Date.now()}`,
            message: `"${deletedBoard.title}" board deleted.`,
            timestamp: Date.now(),
          },
          ...prev,
        ]);
  ...
  return (
    <View style={styles.mainpage}>
      ...
      <View style={styles.quickActionsWrapper}>
        <Text style={styles.quickActionsHeading}>Welcome back 👋</Text>
          ...
          <View style={styles.modalBackground}>
            ...
            <View style={styles.favouritemodalView}>
              ...
              {boards.filter((board) => board.isFavorite).length === 0 ? (
                <Text style={styles.favouriteemptyText}>
                  No Favorite Boards
                </Text>
              ) : (
                boards
            ...
            {recents.length === 0 ? (
              ...
              recents.map((item) => (
                <View key={item.id} style={styles.recentCard}>
                  ...
                  <Text style={styles.recentCardTimestamp}>
            ...
            {inviteSent ? (
              <View style={{ alignItems: "center", marginTop: 24 }}>
                ...
                >
                  ...
                  >
                    <Text style={{ fontWeight: "bold" }}>You</Text> have invited{" "}
                    <Text style={{ fontWeight: "bold" }}>{inviteEmail}</Text> to
                    work on the{" "}
                ...
                >
                  <Text
                    style={{
                      color: theme === "dark" ? "#2C8CFF" : "#0B1F3A",
                      fontWeight: "bold",
                      fontSize: 16,
                    }}
                  >
              ...
              <>
                ...
                />
                {inviteError ? (
                  <Text style={{ color: "#E74C3C", marginBottom: 8 }}>
                    {inviteError}
                  </Text>
                ) : null}
                <TouchableOpacity
